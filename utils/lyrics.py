...
class A_ZLyrics(LyricsPlatform):
        ...
        try:
            ...
            if count > 1:
                ...
                return {
                    lyrics_parts[i]
                    .replace("[", "")
                    .replace(":]", ""): self.clear_text(lyrics_parts[i + 1])
    ...
    async def google_get(self, acc=0.6, artist="", title="") -> Optional[str]:
        ...
        if len(results):
            ...
            if artist:
                jaro_artist = self.jaro_distance(
                    artist.replace(" ", "").lower(), results[0][0].lower()
                )
            if title:
                jaro_title = self.jaro_distance(
                    title.replace(" ", "").lower(), results[0][1].lower()
                )
    ...
    async def get_lyrics(self, title: str, artist: str) -> Optional[dict[str, str]]:
        ...
        try:
            song = await loop.run_in_executor(
                None, lambda: self.genius.search_song(title=title, artist=artist)
            )
            if not song:
                return None
...
class Lrclib(LyricsPlatform):
    ...
    async def get_lyrics(self, title: str, artist: str) -> Optional[dict[str, str]]:
        ...
        if result and len(result) > 0:
            return {
                "default": result[0].get("plainLyrics", "")
                or result[0].get("lyrics", "")
            }
        return None
...
class MusixMatch(LyricsPlatform):
    ...
    async def get_track_lyrics(
        self, track_id: Optional[str] = None, track_isrc: Optional[str] = None
    ) -> dict:
    ...
    async def get_latest_app(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                url, headers={**self.headers, "Cookie": "mxm_bab=AB"}
            ) as response:
    ...
    async def make_request(self, url: str) -> dict:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                signed_url, headers=self.headers, timeout=5
            ) as response:
                if response.status != 200:
                    raise Exception(
                        f"HTTP Error: {response.status} for URL: {signed_url}"
                    )
                try:
    ...
    async def get_lyrics(self, title: str, artist: str) -> Optional[dict[str, str]]:
        try:
            results = await self.search_tracks(
                track_query=f"{artist} {title}" if artist else title
            )
