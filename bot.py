...
# --- GLOBAL CONSTANTS ---
SOURCE_GUILD_ID = int(os.getenv("SOURCE_GUILD_ID", "1413525842490953891"))
SYNC_ROLE_ID = int(os.getenv("SYNC_ROLE_ID", "1413696208920117368"))
...
if getattr(ui_config, "DEV_MODE", False) and not getattr(
    ui_config, "IMPERSONATE_MAIN", False
):
...
class Cyori(commands.Bot):
    ...
    async def build_dashboard_state(self, guild_id: int):
        ...
        try:
            ...
            if not g:
                return {"playing": False, "paused": False, "pos": 0, "len": 0}
                        ...
                        queue_tracks.append(
                            {
                                ...
                                or getattr(t, "id", ""),
                                "duration": getattr(
                                    t, "length", getattr(t, "duration", 0)
                                ),
                                "thumbnail": (
                                    t.thumbnail
                                    if getattr(t, "thumbnail", None)
                                    and "null" not in t.thumbnail
                                    else "logo-circle.png"
                                ),
                            }
                        )
                except Exception:
                ...
                d.update(
                    {
                        ...
                        "duration": (
                            p.current.length if hasattr(p.current, "length") else 0
                        ),
                        ...
                        "thumbnail": (
                            p.current.thumbnail
                            if getattr(p.current, "thumbnail", None)
                            and "null" not in p.current.thumbnail
                            else "logo-circle.png"
                        ),
    ...
    async def on_cytechlink_track_exception(self, player, track, exception):
        ...
        embed = discord.Embed(
            title="🎵 Music Player Error", color=discord.Color.orange()
        )
        embed.add_field(
            name="Track", value=f"[{track.title}]({track.uri})", inline=False
        )
        embed.add_field(
            name="Guild", value=f"{player.guild.name} ({player.guild.id})", inline=True
        )
        ...
        if self.error_webhook_url:
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    await webhook.send(
                        embed=embed,
                        username=f"{self.user.name if self.user else 'Bot'} DJ Watchdog",
    ...
    async def setup_hook(self):
        ...
        async def global_interaction_check(interaction: discord.Interaction):
            ...
            if not interaction.guild:
                if (
                    interaction.command
                    and interaction.command.qualified_name not in dm_whitelist
                ):
                    await interaction.response.send_message(
                        "❌ This command can only be used in a server! / คำสั่งนี้ใช้ได้เฉพาะในเซิร์ฟเวอร์เท่านั้น!",
                        ephemeral=True,
                    )
                    return False
    ...
    async def get_lang(self, guild_id: int) -> str:
        try:
            guild_data = await self.db_manager.get_guild(guild_id)
    ...
    async def register_to_proxy(self, target_url=None):
        ...
        if not proxy_url or DEPRECATED_MODE:
            return
        ...
        payload = {"action": "register_bot", "url": my_url}
        ...
        try:
            ...
            print(
                f"[Proxy] Error registering to {proxy_url}: {e} (Type: {type(e).__name__})"
            )
            ...
            async with self.session.get(url, timeout=5) as resp:
                if resp.status == 200:
                    ...
                    for event_wrap in events:
                        ...
                        if e_id in self._processed_events:
                            continue
            ...
            async with self.session.post(url, json=payload, headers=headers) as resp:
                if resp.status != 200:
                    ...
                    if not DEPRECATED_MODE:
                        print(
                            f"[TopGG Error] Failed to post stats for {self.user.id}: {resp.status} - {text}"
                        )
                else:
                    if not DEPRECATED_MODE:
                        print(
                            f"[TopGG] Successfully posted {len(self.guilds)} servers for {self.user.id}"
                        )
        except Exception as e:
            if not DEPRECATED_MODE:
    ...
    async def is_premium(self, user_id: int, guild_id: int = None) -> bool:
        ...
        try:
            # Check User Premium
            u_data = await self.db_manager.get_user(str(user_id))
    ...
    def none_play_embed(self, lang="en", guild_data: dict = None):
        ...
        vote = (
            f"[Vote](https://top.gg/bot/{self.user.id}/vote)"
            if self.user
            else "[Vote](https://top.gg)"
        )
    ...
    async def on_guild_join(self, guild: discord.Guild):
        ...
        channel = discord.utils.get(guild.text_channels, name="general") or next(
            (
                c
        ...
        if channel:
            ...
            if guild.preferred_locale == "en-US":
                lang = "en"

            from utils.luxury import LuxuryEmbed, luxury_line

            embed = LuxuryEmbed(
                title=f"👋 ขอบคุณที่เชิญ {self.user.name} เข้าสู่เซิร์ฟเวอร์!",
                description=f"ขอบคุณที่ไว้วางใจให้เราดูแลเสียงเพลงในห้องของคุณครับ\n\n"
                f"{luxury_line()}\n"
                f"📌 **วิธีการเริ่มต้น:**\n"
                f"1. ใช้คำสั่ง `/setup` เพื่อสร้างห้องขอเพลงอัตโนมัติ (แนะนำมาก!)\n"
                f"2. หรือพิมพ์คำสั่ง `/play <ชื่อเพลง>` เพื่อเล่นเพลงทันที\n"
                f"3. พิมพ์ `/help` เพื่อดูคำสั่งทั้งหมด\n\n"
                f"💎 **Premium:** สนับสนุนเราเพื่อปลดล็อกฟีเจอร์ 24/7 และคุณภาพเสียงสูงสุด!\n"
                f"{luxury_line()}",
                color=ui_config.EMBED_COLOR,
            )
    ...
    async def update_guild_embed(self, guild_data, guild_id=None):
        if guild_data:
            ...
            try:
                ...
                if guild:
                    is_owner_prem = await self.is_premium(
                        guild.owner_id, guild_id=guild.id
                    )
                    if not is_owner_prem:
                        # Hide premium assets if owner not premium
                        guild_data = guild_data.copy()
                        if "premium_banner" in guild_data:
                            del guild_data["premium_banner"]
                        if "premium_image" in guild_data:
                            del guild_data["premium_image"]
            except Exception:
                pass
            ...
            try:
                ...
                await queue_message.edit(
                    embed=discord.Embed(
                        title=self.i18n.get("no_queue", lang),
                        color=ui_config.EMBED_COLOR,
                    )
                )
                ...
                await play_message.edit(
                    embed=self.none_play_embed(lang, guild_data), view=view
                )
            except Exception:
                pass
    ...
    async def update_all_guild_embeds_on_startup(self):
        try:
            # 🛡️ Use the robust db_manager instead of a raw raw query
            data = await self.db_manager._fetch_root()
        ...
        await asyncio.gather(
            *(sem_task(gid, gdata) for gid, gdata in data["guilds"].items())
        )
    ...
    async def update_stats_task(self):
        try:
            ...
            print(
                f"[Stats] {self.user.name} ({mode}) synced {len(self.guilds)} guilds to Top.gg."
            )
        except Exception as e:
    ...
    async def status_loop(self):
        ...
        try:
            statuses = [
                discord.Activity(
                    type=discord.ActivityType.watching, name=f"{self.user.name if self.user else 'Bot'} | /help"
                ),
                discord.Activity(
                    type=discord.ActivityType.competing,
                    name=f"My name is {self.user.name if self.user else 'CytechMusic'} and I can play music",
                ),
                discord.Activity(
            ...
            await self.change_presence(
                activity=statuses[self._status_index % len(statuses)]
            )
...
\____/    /_/ \____/_/ |_/___/    
    ~ P R E M I U M   G O L D ~
            ...
            print("[🔒 DAVE E2EE] Discord Audio & Voice End-to-End Encryption is ENABLED")
        else:
            print(
                f"[*] {self.user} ({self.user.id}) [OLD BOT] is now running silently (MIGRATION MODE)."
            )
        ...
        try:
            ...
            if DEPRECATED_MODE:
                print(
                    f"[*] Identity: {self.user.id} [MIGRATION MODE] - Skipping command sync."
                )
            else:
                ...
                if dev_mode and test_guild_id:
                    ...
                    print(
                        f"[*] DEV_MODE: Synced {len(sy)} commands to Test Guild ({test_guild_id})"
                    )
                else:
        ...
        if not DEPRECATED_MODE:
            ...
            print(
                f"[*] Identity: {self.user.id} [MIGRATION MODE] - Scanning guilds for proactive migration..."
            )
            for guild in self.guilds:
                ...
                async def check_and_migrate(g):
                    try:
                        ...
                        has_new_bot = g.get_member(
                            int(NEW_BOT_ID)
                        ) or await g.fetch_member(int(NEW_BOT_ID))
                        if has_new_bot:
    ...
    async def sync_community_roles_task(self):
        ...
        if not source_guild:
            try:
                source_guild = await self.fetch_guild(SOURCE_GUILD_ID)
            except Exception:
                return
                    ...
                    try:
                        if should_have_role and not has_role:
                            await member.add_roles(
                                role, reason="Cyori Audit: Member + Premium Sync"
                            )
                        elif not should_have_role and has_role:
                            await member.remove_roles(
                                role, reason="Cyori Audit: Not Premium/Source anymore"
                            )
                    except Exception:
                        pass
            except Exception:
    ...
    async def check_deprecation(self, interaction_or_msg):
        ...
        # 2. Handle Prefix Messages (Only if it's a command or mention)
            ...
            if self.user in interaction_or_msg.mentions:
                ...
                guild_id = (
                    str(interaction_or_msg.guild.id)
                    if interaction_or_msg.guild
                    else None
                )
                ...
                try:
                    ...
                    if guild_id and str(guild_id) in data.get("guilds", {}):
                        prefix = data["guilds"][str(guild_id)].get(
                            "prefix", DEFAULT_PREFIX
                        )
                except Exception:
                    pass
        ...
        try:
            guild_id = (
                str(interaction_or_msg.guild.id) if interaction_or_msg.guild else None
            )
            data = await collection_myasync.find_one({}) or {}
            lang = (
                data.get("guilds", {}).get(guild_id, {}).get("lang", "en")
                if guild_id
                else "en"
            )
        except Exception:
        ...
        title = (
            "≡ƒÜ¿ Cyori has specialized to a NEW version!"
            if lang == "en"
            else "≡ƒÜ¿ Cyori α╕óα╣ëα╕▓α╕óα╣äα╕¢α╣Çα╕ºα╕¡α╕úα╣îα╕èα╕▒α╕Öα╣âα╕½α╕íα╣êα╣üα╕Ñα╣ëα╕º!"
        )
        desc = (
            ...
            if lang == "en"
            else "α╕Üα╕¡α╕ùα╕òα╕▒α╕ºα╣Çα╕üα╣êα╕▓α╕Öα╕╡α╣ë **α╣Çα╕Ñα╕┤α╕üα╣âα╕èα╣ëα╕çα╕▓α╕Öα╣üα╕Ñα╣ëα╕º** α╣Çα╕₧α╕╖α╣êα╕¡α╕üα╕▓α╕úα╕ƒα╕▒α╕çα╣Çα╕₧α╕Ñα╕çα╕ùα╕╡α╣êα╕Ñα╕╖α╣êα╕Öα╕éα╕╢α╣ëα╕Öα╣üα╕Ñα╕░α╣Çα╕¬α╕ûα╕╡α╕óα╕úα╕üα╕ºα╣êα╕▓α╣Çα╕öα╕┤α╕í α╕úα╕Üα╕üα╕ºα╕Öα╕óα╣ëα╕▓α╕óα╣äα╕¢α╣âα╕èα╣ëα╕Üα╕¡α╕ùα╕òα╕▒α╕ºα╣âα╕½α╕íα╣êα╣üα╕ùα╕Öα╕Öα╕░α╕äα╕úα╕▒α╕Ü!\n\n"
        ...
        embed = discord.Embed(
            title=title, description=desc, color=0xFF0000  # Red for alert
        )
        embed.set_image(
            url="https://i.postimg.cc/qBGwRzwN/wmremove-transformed-(1).jpg"
        )
        ...
        try:
            if isinstance(interaction_or_msg, discord.Interaction):
                if not interaction_or_msg.response.is_done():
                    await interaction_or_msg.response.send_message(
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            data = (
                await asyncio.wait_for(collection_myasync.find_one({}), timeout=5) or {}
            )
        except asyncio.TimeoutError:
            data = {"guilds": {}}
        ...
        if message.content in [f"<@{self.user.id}>", f"<@!{self.user.id}>"]:
            async with message.channel.typing():
                ...
                embed = discord.Embed(
                    ...
                    description=self.i18n.get(
                        "hello_desc", lang, author=message.author, prefix=prefix
                    ),
                    color=ui_config.EMBED_COLOR,
                )
                embed.set_thumbnail(
                ...
                embed.set_footer(
                    text=self.i18n.get(
                        "bot_version_footer", lang, bot_name=self.user.name, version=self.bot_version
                    )
                )
    ...
    async def log_guild_event(self, guild: discord.Guild, action: str):
        try:
            ...
            webhooks = [
                webhook for webhook in webhooks if webhook.name == "Cytech Could Logs"
            ]
            ...
            if webhooks:
                ...
                try:
                    longest_invite = max(
                        invites,
                        key=lambda inv: (
                            inv.max_age if inv.max_age != 0 else float("inf")
                        ),
                        default=None,
                    )
                except Exception:
                ...
                embed = (
                    discord.Embed(
                        title=f"{guild.name}'s Information", color=ui_config.EMBED_COLOR
                    )
                    .set_author(
                        name=f"Guild {action}",
                        icon_url=(
    ...
    async def perform_migration_and_leave(self, guild: discord.Guild):
        ...
        if not DEPRECATED_MODE:
            print(
                f"[*] Migrating Guild: {guild.name} ({guild.id}) -> Handing over to New Bot."
            )
            ...
            if guild_data:
                ...
                if channel:
                    ...
                    try:
                        ...
                        if not DEPRECATED_MODE:
                            print(
                                f"[*] Deleted dedicated channel {channel_id} in {guild.name}"
                            )
                    except Exception:
        ...
        if target_channel:
            try:
                ...
                title = (
                    "≡ƒæï α╕óα╣ëα╕▓α╕óα╕éα╣ëα╕¡α╕íα╕╣α╕Ñα╣Çα╕¬α╕úα╣çα╕êα╕¬α╕┤α╣ëα╕Ö!"
                    if lang != "en"
                    else "≡ƒæï Migration Complete!"
                )
                desc = (
                    (
    ...
    async def on_member_join(self, member):
        ...
        if not DEPRECATED_MODE:
            ...
            if source_guild:
                try:
                    is_in_source = source_guild.get_member(
                        member.id
                    ) or await source_guild.fetch_member(member.id)
                except Exception:
                    is_in_source = False
            ...
            if role:
                try:
                    if is_in_source and is_prem:
                        await member.add_roles(
                            role, reason="Cyori Sync: Member + Premium"
                        )
                    else:
                        if role in member.roles:
                            await member.remove_roles(
                                role,
                                reason="Cyori Sync: Missing requirement (Source/Pre)",
                            )
                except Exception:
                    pass
    ...
    async def on_member_remove(self, member):
        ...
        if member.guild.id == SOURCE_GUILD_ID:
            print(
                f"[*] Member {member} left source guild. Removing roles across servers..."
            )
            for guild in self.guilds:
                if guild.id == SOURCE_GUILD_ID:
                    continue
                ...
                if target_member and role in target_member.roles:
                    try:
                        await target_member.remove_roles(
                            role, reason="Cyori Sync: Left Source Guild"
                        )
                    except Exception:
                        pass
    ...
    async def on_guild_join(self, guild: discord.Guild):
        ...
        if DEPRECATED_MODE:
            try:
                # Use cache or fetch
                has_new_bot = guild.get_member(
                    int(NEW_BOT_ID)
                ) or await guild.fetch_member(int(NEW_BOT_ID))
                if has_new_bot:
                    await self.perform_migration_and_leave(guild)
            except Exception:
                pass
