...
class Cyori(commands.Bot):
    ...
    async def log_error(self, error, ctx=None, event_name=None):
        ...
        if not self.error_log_channel_id and not self.error_webhook_url:
            return
        ...
        if self.error_webhook_url:
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    await webhook.send(embed=embed, username="Cyori Bug Hunter", avatar_url=self.user.display_avatar.url if self.user else None)
                    return # Success
    ...
    async def on_command_error(self, ctx: commands.Context, error: commands.CommandError):
        ...
        if isinstance(error, commands.MissingPermissions):
            lang = await self.get_lang(ctx.guild.id)
            perms = ", ".join(error.missing_permissions)
            embed = discord.Embed(
                title=self.i18n.get("error_title", lang) or "Error",
    ...
    async def get_lang(self, guild_id: int) -> str:
        try:
            ...
            if "guilds" in data and str(guild_id) in data["guilds"]:
                return data["guilds"][str(guild_id)].get("lang", "en")
        except Exception:
            pass
        return "en"
    ...
    async def is_premium(self, user_id: int, guild_id: int = None) -> bool:
        ...
        try:
            ...
            if not data:
                # Fallback to general find_one
                data = await collection_myasync.find_one({}) or {}
            
            # Check User Premium
            u_data = data.get("users", {}).get(str(user_id), {})
        ...
        if channel:
            ...
            embed = LuxuryEmbed(
                ...
                description=f"ขอบคุณที่ไว้วางใจให้เราดูแลเสียงเพลงในห้องของคุณครับ\n\n"
                            ...
                            f"2. หรือสิมรณ์คำสั่ง `/play <ชื่อเพลง>` เพื่อเล่นเเพลงทันที\n"
                            f"3. พิมพ์ `/help` เพื่อดูคำสั่งทั้งหมด\n\n"
                            f"💎 **Premium:** สนับสนุนเราเพื่อปลดล็อคฟีเจอร์ 24/7 และคุณภาพเสียงสูงสุด!\n"
                            f"{luxury_line()}",
                color=ui_config.EMBED_COLOR
            )
...
\____/    /_/ \____/_/ |_/___/    
    ...
    async def check_deprecation(self, interaction_or_msg):
        ...
        title = "🚨 Cyori has specialized to a NEW version!" if lang == "en" else "🚨 Cyori ย้ายไปเวอร์ชันใหม่แล้ว!"
        desc = (
            ...
            "**[👉 CLICK HERE TO INVITE THE NEW CYORI]({invite})**\n\n"
            "*Your premium and playlists will still work on the new bot.*"
            if lang == "en" else
            "บอทตัวเก่านี้ **เลิกใช้งานแล้ว** เพื่อการฟังเพลงที่ลื่นขึ้นและเสถียรกว่าเดิม รบกวนย้ายไปใช้บอทตัวใหม่แทนนะครับ!\n\n"
            "**[👉 คลิกที่นี่เพื่อเชิญ Cyori ตัวใหม่]({invite})**\n\n"
            "*พรีเมียมและเพลย์ลิสต์ของคุณจะตามไปด้วยอัตโนมัติ*"
        ...
        embed.set_footer(text="Cyori Migration System • 2026")
    ...
    async def perform_migration_and_leave(self, guild: discord.Guild):
        ...
        if target_channel:
            try:
                ...
                title = "👋 ย้ายข้อมูลเสร็จสิ้น!" if lang != "en" else "👋 Migration Complete!"
                desc = (
                    f"บอทตัวเก่าได้ส่งมอบหน้าที่ให้ <@{NEW_BOT_ID}> เรียบร้อยแล้ว\n"
                    "ตอนนี้คุณสามารถใช้งานบอทตัวใหม่ได้ทันทีครับ!\n\n"
                    "*บอทตัวเก่ากำลังออกจากเซิร์ฟเวอร์...*"
                ) if lang != "en" else (
