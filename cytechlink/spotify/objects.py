class Track:
    ...
    __slots__ = ("name", "artists", "artist_id", "length", "id", "image", "uri")
    ...
    def __init__(self, data: dict, image=None) -> None:
        ...
        self.image: str = (
            images[0]["url"]
            if (images := data.get("album", {}).get("images"))
            else image
        )
    ...
    def __repr__(self) -> str:
        return (
            f"<Cytechlink.spotify.Track name={self.name} artists={self.artists} "
...
class Album:
    ...
    __slots__ = ("name", "artists", "image", "tracks", "total_tracks", "id", "uri")
    ...
    def __repr__(self) -> str:
        return (
            f"<Cytechlink.spotify.Album name={self.name} artists={self.artists} id={self.id} "
...
class Artist:
    ...
    __slots__ = ("tracks", "image", "total_tracks", "owner", "id", "uri", "name")
    ...
    def __init__(self, data: dict) -> None:
        ...
        if self.tracks:
            ...
            self.uri: str = data["tracks"][0]["album"]["artists"][0]["external_urls"][
                "spotify"
            ]
    ...
    def __repr__(self) -> str:
        return (
            f"<Cytechlink.spotify.Artist name={self.name} owner={self.owner} id={self.id} "
            f"total_tracks={self.total_tracks} tracks={self.tracks}>"
        )
...
class Playlist:
    ...
    __slots__ = ("name", "tracks", "owner", "total_tracks", "id", "image", "uri")
    ...
    def __init__(self, data: dict, tracks: list[Track]) -> None:
        ...
        self.id: str = data.get("id")
        self.image: str = (
            data["images"][0]["url"] if len(data.get("images", [])) else None
        )
        self.uri: str = data["external_urls"]["spotify"]

    def __repr__(self) -> str:
        return (
            f"<Cytechlink.spotify.Playlist name={self.name} owner={self.owner} id={self.id} "
            f"total_tracks={self.total_tracks} tracks={self.tracks}>"
        )
