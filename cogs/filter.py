...
class Filter(commands.Cog):
    ...
    async def _get_player_or_error(
    ...
    async def effect_autocomplete(
        ...
        if current:
            return [
                ...
                if current.lower() in effect.tag.lower()
            ]
        return [
            app_commands.Choice(name=effect.tag, value=effect.tag) for effect in filters
        ]
    ...
    @app_commands.describe(
        value="Speed multiplier (e.g. 1.25) / ตัวคูณความเร็ว (เช่น 1.25)"
    )
    async def speed(
        self, ctx: commands.Context, value: commands.Range[float, 0.1, 2.0]
    ):
        ...
        async with ctx.typing():
            ...
            lang = await self.bot.get_lang(ctx.guild.id)
            await ctx.reply(
                self.bot.i18n.get("filter_speed", lang, value=value), delete_after=7
            )
    ...
    async def karaoke(
        ...
        async with ctx.typing():
            ...
            await ctx.reply(
                self.bot.i18n.get(
                    "filter_karaoke", lang, level=level, monolevel=monolevel
                ),
                delete_after=7,
            )
    ...
    async def tremolo(
        ...
        async with ctx.typing():
            ...
            await player.add_filter(
                cytechlink.Tremolo(tag="tremolo", frequency=frequency, depth=depth)
            )
            lang = await self.bot.get_lang(ctx.guild.id)
            await ctx.reply(
                self.bot.i18n.get(
                    "filter_tremolo", lang, frequency=frequency, depth=depth
                ),
                delete_after=7,
            )
    ...
    async def vibrato(
        ...
        async with ctx.typing():
            ...
            await player.add_filter(
                cytechlink.Vibrato(tag="vibrato", frequency=frequency, depth=depth)
            )
            lang = await self.bot.get_lang(ctx.guild.id)
            await ctx.reply(
                self.bot.i18n.get(
                    "filter_vibrato", lang, frequency=frequency, depth=depth
                ),
                delete_after=7,
            )
    ...
    async def rotation(
        self, ctx: commands.Context, hertz: commands.Range[float, 0.01, 2.0] = 0.2
    ) -> None:
        ...
        async with ctx.typing():
            ...
            await player.add_filter(
                cytechlink.Rotation(tag="rotation", rotation_hertz=hertz)
            )
            lang = await self.bot.get_lang(ctx.guild.id)
            await ctx.reply(
                self.bot.i18n.get("filter_rotation", lang, hertz=hertz), delete_after=7
            )
    ...
    async def distortion(self, ctx: commands.Context) -> None:
        ...
        async with ctx.typing():
            ...
            lang = await self.bot.get_lang(ctx.guild.id)
            await ctx.reply(
                self.bot.i18n.get("filter_distortion", lang), delete_after=7
            )
    ...
    @app_commands.describe(
        smoothing="Smoothing level (default 20.0) / ระดับความนุ่มนวล (ปกติ 20.0)"
    )
    async def lowpass(
        self, ctx: commands.Context, smoothing: commands.Range[float, 10.0, 30.0] = 20.0
    ) -> None:
        ...
        async with ctx.typing():
            ...
            await player.add_filter(
                cytechlink.LowPass(tag="lowpass", smoothing=smoothing)
            )
            lang = await self.bot.get_lang(ctx.guild.id)
            await ctx.reply(
                self.bot.i18n.get("filter_lowpass", lang, smoothing=smoothing),
                delete_after=7,
            )
    ...
    @app_commands.describe(
        effect="Specific effect name to remove / ชื่อเอฟเฟกต์ที่ต้องการลบ"
    )
    ...
    async def cleareffect(self, ctx: commands.Context, effect: str = None) -> None:
        ...
        async with ctx.typing():
            ...
            if effect:
                ...
                await ctx.reply(
                    self.bot.i18n.get("filter_removed", lang, effect=effect),
                    delete_after=7,
                )
            else:
                await player.reset_filter()
                await ctx.reply(
                    self.bot.i18n.get("filter_cleared", lang), delete_after=7
                )
