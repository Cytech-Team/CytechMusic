...
class Owner(commands.Cog):
    ...
    @commands.hybrid_group(
        name="own",
        description="Owner-only management commands / คำสั่งจัดการสำหรับเจ้าของบอท",
    )
    @commands.is_owner()
    ...
    # ===================================================================
    @own_group.command(
        name="shutdown", description="Shut down the bot / ปิดการทำงานของบอท"
    )
    @commands.is_owner()
    ...
    # ===================================================================
    @own_group.command(
        name="reload", description="Reload a cog / รีโหลดส่วนเสริม (Cog)"
    )
    @commands.is_owner()
    ...
    @own_group.command(
        name="unload", description="Unload a cog / ยกเลิกโหลดส่วนเสริม (Cog)"
    )
    @commands.is_owner()
    ...
    # ===================================================================
    @own_group.command(
        name="sync", description="Sync slash commands / ซิงค์คำสั่ง Slash Command"
    )
    @commands.is_owner()
    ...
    @own_group.command(
        name="servers",
        description="List top guilds by member count / แสดงรายการเซิร์ฟเวอร์เรียงตามจำนวนสมาชิก",
    )
    @commands.is_owner()
    ...
    @own_group.command(
        name="leave",
        description="Force bot to leave a guild / บังคับบอทออกจากเซิร์ฟเวอร์",
    )
    @commands.is_owner()
    ...
    @own_group.command(
        name="say",
        description="Make the bot send a message to a channel / ให้บอทส่งข้อความไปยังห้องที่กำหนด",
    )
    @commands.is_owner()
    async def say(
    ...
    # ===================================================================
    @own_group.command(
        name="dm", description="DM the server owner / ส่งข้อความหาเจ้าของเซิร์ฟเวอร์"
    )
    @commands.is_owner()
    async def dmowner(self, ctx: commands.Context, *, message: str):
        ...
        try:
            ...
            await ctx.send(
                f"Cannot DM {owner} — they have DMs disabled or blocked the bot.",
                ephemeral=True,
            )
    ...
    @own_group.command(
        name="announce",
        description="Send a DM to all server owners / ประกาศถึงเจ้าของเซิร์ฟเวอร์ทุกคน",
    )
    @commands.is_owner()
    async def announce(self, ctx: commands.Context, *, message: str):
        ...
        if len(message) > 1800:
            return await ctx.send(
                "Message too long! Max 1800 characters.", ephemeral=True
            )
    ...
    # ===================================================================
    @own_group.command(
        name="status", description="Change bot status / เปลี่ยนสถานะของบอท"
    )
    @commands.is_owner()
    @app_commands.describe(
        status_type="Type: playing, listening... / ประเภทสถานะ",
        text="Status text / ข้อความสถานะ",
    )
    ...
    async def setstatus(self, ctx: commands.Context, status_type: str, *, text: str):
        ...
        try:
            activity_type = getattr(
                discord.ActivityType, status_type, discord.ActivityType.playing
            )
            ...
            await ctx.send(
                f"Status changed to: `{status_type.capitalize()} {text}`",
                ephemeral=True,
            )
        except Exception as e:
    ...
    @own_group.command(
        name="rename", description="Change bot username / เปลี่ยนชื่อผู้ใช้ของบอท"
    )
    @commands.is_owner()
    ...
    @own_group.command(
        name="avatar", description="Change bot avatar / เปลี่ยนรูปโปรไฟล์ของบอท"
    )
    @commands.is_owner()
    async def setavatar(self, ctx: commands.Context, url: str = None):
        ...
        if not url:
            return await ctx.send(
                "Please provide a URL or attach an image.", ephemeral=True
            )
