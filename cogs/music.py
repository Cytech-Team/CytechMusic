...
class page(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user != self.author:
            ...
            await interaction.followup.send(
                content=self.bot.i18n.get("perm_button", lang),
                ephemeral=True,
                delete_after=7,
            )
            return False
    ...
    async def update_page(self):
        ...
        embed.set_footer(
            text=self.bot.i18n.get(
                "page_footer", lang, current=self.index + 1, total=len(self.source)
            ),
    ...
    @discord.ui.button(
        label="Lyrics", style=discord.ButtonStyle.secondary, custom_id="lyrics_btn"
    )
...
class Music(commands.Cog):
    ...
    async def on_cytechlink_track_end(
        self, player: cytechlink.Player, track: cytechlink.Track, _
    ):
        await player.do_next()

    @commands.Cog.listener()
    async def on_cytechlink_track_stuck(
        self, player: cytechlink.Player, track: cytechlink.Track, _
    ):
    ...
    @commands.Cog.listener()
    async def on_cytechlink_track_exception(
        self, player: cytechlink.Player, track: cytechlink.Track, _
    ):
        ...
        if player.current and player.current.track_id == track.track_id:
            ...
            try:
                ...
                await player.context.send(
                    self.bot.i18n.get("error_next_song_10s", lang), delete_after=10
                )
            except Exception:
                pass
            await player.do_next()
    ...
    async def _play(self, interaction: discord.Interaction, message: discord.Message):
        ...
        if not vc:
            msg = await interaction.followup.send(
                self.bot.i18n.get("error_voice_required", lang)
            )
        ...
        if not results:
            msg = await interaction.followup.send(
                self.bot.i18n.get("error_no_results", lang)
            )
            await asyncio.sleep(7)
            await msg.delete()
            return
        ...
        try:
            if isinstance(results, cytechlink.Playlist):
                if results.tracks:
                    ...
                    msg = await interaction.followup.send(
                        self.bot.i18n.get("playlist_empty", lang)
                    )
                    await asyncio.sleep(7)
                    await msg.delete()
                    return
        ...
        try:
            desc = ""
            if isinstance(results, cytechlink.Playlist):
                desc = self.bot.i18n.get(
                    "added_playlist", lang, name=results.name, count=results.track_count
                )
                if results.uri:
                    desc = f"[{results.name}]({results.uri}) {self.bot.i18n.get('queue_title_added', lang)}"
            else:
                desc = self.bot.i18n.get(
                    "added_to_queue", lang, title=f"[{track.title}]({track.uri})"
                )
            ...
            embes.set_author(
                name=self.bot.i18n.get("queue_title_added", lang),
        ...
        if not player.is_playing:
            try:
                ...
                msg = await interaction.followup.send(
                    self.bot.i18n.get("error_playback", lang)
                )
                await asyncio.sleep(7)
                await msg.delete()
        else:
            ...
            if channel_id and message.channel.id == channel_id:
                async with message.channel.typing():
                    ...
                    try:
                        ...
                        return await ctx.send(
                            self.bot.i18n.get("error_no_results", lang), delete_after=7
                        )
                    ...
                    try:
                        ...
                        return await ctx.send(
                            self.bot.i18n.get("queue_full", lang), delete_after=7
                        )
                    ...
                    embes.set_footer(
                        text=self.bot.i18n.get(
                            "requested_by", lang, user=ctx.author.name
                        ),
    ...
    @app_commands.describe(
        channel="Voice channel to join / ช่องเสียงที่ต้องการให้บอทเข้า"
    )
    async def join(
        ...
        async with ctx.typing():
            ...
            if not ctx.voice_client:
                ...
                await ctx.reply(
                    self.bot.i18n.get("joined_voice", lang, channel=channel.mention),
                    delete_after=7,
                )
            else:
                await ctx.reply(
                    self.bot.i18n.get(
                        "already_connected", lang, channel=ctx.voice_client.channel
                    ),
                    delete_after=7,
                )
    ...
    async def leave(self, ctx: commands.Context):
        ...
        async with ctx.typing():
            ...
            if not player:
                return await ctx.reply(
                    self.bot.i18n.get("error_voice_required", lang), delete_after=7
                )

            if await player.is_privileged(ctx.author):
                await player.destroy()
                return await ctx.reply(
                    self.bot.i18n.get("left_channel", lang), delete_after=7
                )
            return await ctx.reply(
                self.bot.i18n.get("dj_required", lang), delete_after=7
            )
    ...
    async def play_autocomplete(self, interaction: discord.Interaction, current: str):
        ...
        for song in user_history[:25]:
            ...
            if len(val) > 100:
                val = val[:100]
            choices.append(
                app_commands.Choice(name="🕒 " + shorten(song["title"]), value=val)
            )

        if current:
            try:
                tracks = await self.bot.cytech.get_node().get_tracks(
                    f"ytsearch:{current}", requester=interaction.user
                )
                if not tracks:
                    tracks = []
            except Exception:
    ...
    @app_commands.describe(
        search="Song name or link to play / ชื่อเพลงหรือลิงก์ youtube"
    )
    ...
    async def play(self, ctx: commands.Context, *, search: str):
        ...
        try:
            ...
            if search_fallback:
                try:
                    results = await player.get_tracks(
                        search_fallback, requester=ctx.author
                    )
                except Exception:
                    return await ctx.reply(
                        self.bot.i18n.get("error_no_results", lang), delete_after=7
        ...
        try:
            if isinstance(results, cytechlink.Playlist):
                if results.tracks:
                    ...
                    return await ctx.reply(
                        self.bot.i18n.get("playlist_empty", lang), delete_after=7
                    )
            else:
                track = results[0]
                await player.add_track(track)
        ...
        embes.set_thumbnail(url=self.bot.user.avatar.url)
        embes.set_footer(
            text=self.bot.i18n.get("requested_by", lang, user=ctx.author.name),
    ...
    async def nowplaying(self, ctx: commands.Context):
        ...
        if not player or not player.is_playing:
            return await ctx.send(
                self.bot.i18n.get("nothing_playing", lang), ephemeral=True
            )
        ...
        embed = discord.Embed(
            description="Loading player...", color=ui_config.EMBED_COLOR
        )
    ...
    class VolumeModal(discord.ui.Modal):
        def __init__(self, bot, ctx, lang="en"):
            ...
            self.vol = discord.ui.TextInput(
                label=label, min_length=1, max_length=3, required=True, placeholder="50"
            )
        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            if not player or not interaction.guild.voice_client:
                return await interaction.response.send_message(
                    self.bot.i18n.get("error_vol_not_playing", lang), ephemeral=True
                )

            if interaction.user not in player.channel.members:
                return await interaction.response.send_message(
                    self.bot.i18n.get("error_same_channel", lang), ephemeral=True
                )
            ...
            try:
                ...
                return await interaction.response.send_message(
                    self.bot.i18n.get("volume_invalid", lang), ephemeral=True
                )
            ...
            if not 0 < volume <= max_vol:
                if volume > 100 and max_vol == 100:
                    return await interaction.response.send_message(
                        self.bot.i18n.get("premium_volume_limit", lang), ephemeral=True
                    )
                return await interaction.response.send_message(
                    self.bot.i18n.get("volume_invalid", lang), ephemeral=True
                )
            ...
            await interaction.response.send_message(
                self.bot.i18n.get("volume_set", lang, volume=volume), delete_after=7
            )
    ...
    async def volume(self, ctx: commands.Context, volume: int = None):
        ...
        if volume is None:
            if ctx.interaction:
                ...
                lang = await self.bot.get_lang(ctx.guild.id)
                return await ctx.reply(
                    self.bot.i18n.get("volume_invalid", lang), delete_after=7
                )
        ...
        async with ctx.typing():
            ...
            if not ctx.voice_client:
                return await ctx.reply(
                    self.bot.i18n.get("error_vol_not_playing", lang), delete_after=7
                )
            ...
            if (
                ...
                return await ctx.reply(
                    self.bot.i18n.get("error_same_channel", lang), delete_after=7
                )
            ...
            if not 0 < vol <= max_vol:
                if vol > 100 and max_vol == 100:
                    return await ctx.reply(
                        self.bot.i18n.get("premium_volume_limit", lang), delete_after=7
                    )
                return await ctx.reply(
                    self.bot.i18n.get("volume_invalid", lang), delete_after=7
                )
            ...
            if await player.is_privileged(ctx.author):
                ...
                await self._notify_web_dashboard(ctx.guild.id)
                return await ctx.reply(
                    self.bot.i18n.get("volume_set", lang, volume=vol), delete_after=7
                )
            return await ctx.reply(
                self.bot.i18n.get("dj_required", lang), delete_after=7
            )
    ...
    async def pause(self, ctx: commands.Context):
        ...
        async with ctx.typing():
            ...
            if not ctx.voice_client:
                return await ctx.reply(
                    self.bot.i18n.get("error_pause_not_playing", lang), delete_after=7
                )
    ...
    @app_commands.describe(
        index="Index of song to skip to (optional) / ลำดับเพลงที่ต้องการข้ามไป (ไม่จำเป็น)"
    )
    async def skip(self, ctx: commands.Context, index: int = 0):
        ...
        async with ctx.typing():
            ...
            if not ctx.voice_client:
                return await ctx.reply(
                    self.bot.i18n.get("error_skip_not_playing", lang), delete_after=7
                )
            ...
            if await player.is_privileged(ctx.author):
                if index:
                    try:
                        ...
                        return await ctx.reply(
                            self.bot.i18n.get("invalid_song_index", lang),
                            delete_after=7,
                        )
                ...
                await self._notify_web_dashboard(ctx.guild.id)
                return await ctx.reply(
                    self.bot.i18n.get("skipped", lang, author=ctx.author),
                    delete_after=7,
                )
            return await ctx.reply(
                self.bot.i18n.get("dj_required", lang), delete_after=7
    ...
    @app_commands.describe(
        index="Index of the song to skip to / ลำดับเพลงที่ต้องการข้ามไป"
    )
    ...
    async def clear(self, ctx: commands.Context):
        ...
        if not player or player.queue.is_empty:
            return await ctx.reply(
                self.bot.i18n.get("queue_empty", lang), delete_after=7
            )
    ...
    async def stop(self, ctx: commands.Context):
        ...
        async with ctx.typing():
            ...
            if not ctx.voice_client:
                return await ctx.reply(
                    self.bot.i18n.get("error_stop_not_playing", lang), delete_after=7
                )
            ...
            if await player.is_privileged(ctx.author):
                ...
                await self._notify_web_dashboard(ctx.guild.id)
                return await ctx.reply(
                    self.bot.i18n.get("stopped", lang), delete_after=10
                )
            return await ctx.reply(
                self.bot.i18n.get("dj_required", lang), delete_after=7
            )
    ...
    @app_commands.describe(
        position="Time position (e.g. 1:20) / ตำแหน่งเวลา (เช่น 1:20)"
    )
    async def seek(self, ctx: commands.Context, position: str):
        ...
        if num is None:
            return await ctx.send(
                self.bot.i18n.get("invalid_time_format", lang), delete_after=7
            )
    ...
    async def queue(self, ctx: commands.Context):
        ...
        async with ctx.typing():
            ...
            if not player:
                return await ctx.reply(
                    self.bot.i18n.get("no_player_found", lang), delete_after=7
                )
            ...
            if len(queue) > 24:
                ...
                for i in range(0, len(queue), 24):
                    ...
                    for i, track in enumerate(paginated_queue, start=i + 1):
                        try:
                            embed.add_field(
                                name="",
                                value=f"**{i}.** [{track.title}]({track.uri})\n",
                                inline=False,
                            )
                        except Exception:
                            embed.add_field(
                    ...
                    try:
                        title_text = tracks.title.encode(
                    ...
                    embed.add_field(
                        name=self.bot.i18n.get("now_playing_field", lang),
                        value=now_text,
                        inline=True,
                    )
                    embed.set_footer(
                ...
                try:
                    title_text = tracks.title.encode("utf-16", "surrogatepass").decode(
                        "utf-16"
                    )
                except Exception:
                    title_text = tracks.title
                ...
                embed.set_footer(
                    text=self.bot.i18n.get("total_songs", lang, count=len(queue)),
                    icon_url=str(self.bot.user.avatar.url),
                )
    ...
    async def fix_region(self, ctx: commands.Context):
        ...
        async with ctx.typing():
            ...
            if not channel.permissions_for(ctx.guild.me).manage_channels:
                return await ctx.reply(
                    self.bot.i18n.get("error_perm_manage_channels", lang),
                    delete_after=7,
                )

            current_region = (
                str(channel.rtc_region) if channel.rtc_region else "automatic"
            )
            ...
            try:
                ...
                await ctx.reply(
                    self.bot.i18n.get(
                        "region_fixed", lang, channel=channel.name, region=new_region
                    ),
                    delete_after=10,
                )
            except Exception as e:
                await ctx.reply(
                    self.bot.i18n.get("region_fail", lang, e=e), delete_after=7
                )
        ...
        async with ctx.typing():
            ...
            try:
                ...
                if removed:
                    ...
                    await ctx.reply(
                        self.bot.i18n.get("removed_from_queue", lang, title=title),
                        delete_after=7,
                    )
                    await player.update_controller()
                    await self._notify_web_dashboard(ctx.guild.id)
                    ...
                    await ctx.reply(
                        self.bot.i18n.get("invalid_song_index", lang), delete_after=7
                    )
            except Exception:
    ...
    async def shuffle(self, ctx: commands.Context):
        ...
        async with ctx.typing():
            ...
            if not await player.is_privileged(
                ctx.author
            ):  # Use standard check (supports vote if implemented in future, but shuffle usually strict or vote)
    ...
    @app_commands.describe(
        query="Song to search lyrics for (optional) / ชื่อเพลงที่ต้องการค้นหาเนื้อร้อง"
    )
    async def lyrics(self, ctx: commands.Context, *, query: str = None):
        ...
        if not lyrics_data:
            search_url = (
                f"https://www.google.com/search?q={query.replace(' ', '+')}+lyrics"
            )
            embed = discord.Embed(
        ...
        for i, page_text in enumerate(pages):
            embed = discord.Embed(
                title=self.bot.i18n.get("lyrics_title", lang, title=query)
                + (f" (Page {i+1})" if len(pages) > 1 else ""),
    ...
    async def grab(self, ctx: commands.Context):
        ...
        embed.add_field(
            name="Title", value=f"[{track.title}]({track.uri})", inline=False
        )
        ...
        try:
            ...
            await ctx.send(
                "❌ I cannot DM you. Please check your privacy settings.",
                ephemeral=True,
            )
    ...
    async def removedupes(self, ctx: commands.Context):
        ...
        if not player or player.queue.is_empty:
            return await ctx.send(
                self.bot.i18n.get("queue_empty", lang), ephemeral=True
            )
        ...
        await ctx.send(
            f"✅ Removed **{removed_count}** duplicate songs from the queue."
        )

    @commands.hybrid_command(name="move", aliases=["mv"])
    @app_commands.describe(
        from_index="Index of the song to move / ลำดับเพลงที่ต้องการย้าย",
        to_index="New position / ตำแหน่งใหม่",
    )
    async def move(self, ctx: commands.Context, from_index: int, to_index: int):
        ...
        if not (1 <= from_index <= queue_len) or not (1 <= to_index <= queue_len):
            return await ctx.send(
                "❌ Invalid index. / ลำดับเพลงไม่ถูกต้อง", ephemeral=True
            )

        if from_index == to_index:
            return await ctx.send(
                "❌ Positions are the same. / ตำแหน่งเหมือนเดิม", ephemeral=True
            )
    ...
    async def history(self, ctx: commands.Context):
        ...
        if not history_queue:
            return await ctx.send(
                self.bot.i18n.get("history_empty", lang), ephemeral=True
            )

        embed = discord.Embed(
            title=self.bot.i18n.get("history_title", lang), color=ui_config.EMBED_COLOR
        )
